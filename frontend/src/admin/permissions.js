...
export const ADMIN_MENUS = [
  ...
  { key: 'users', label: '用户管理', path: '/admin-console/users', icon: User, permission: 'can_manage_users' },
  {
    key: 'content',
    label: '内容配置',
    path: '/admin-console/content',
    icon: Setting,
    permission: ['can_manage_products', 'can_manage_settings'],
  },
  { key: 'logs', label: '操作日志', path: '/admin-console/logs', icon: Document, permission: 'can_view_logs' },
]

export const canUseAdminAction = (session, permission) => {
  if (Array.isArray(permission)) {
    return permission.some((item) => canUseAdminAction(session, item))
  }
